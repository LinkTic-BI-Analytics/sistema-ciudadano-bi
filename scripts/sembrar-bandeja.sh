...
#   2. sin lugar ninguno    → el caso duro: «en mi casa»
...
#   6. lugar sin municipio  → dijo dónde, pero no lleva a ningún código
#
...
nuevos as (
  insert into participacion.aporte
    (proceso_id, clave_envio, relato_original, canal, lugar_declarado, afectados, desde_cuando,
     es_colectivo, colectivo_declarado, recibido_en)
  select proc.id, 'muestra-' || t.n, t.relato, t.canal, t.lugar, t.afectados, t.desde,
         t.colectivo, t.nombre_grupo, now() - (t.n || ' hours')::interval
  from proc, (values
